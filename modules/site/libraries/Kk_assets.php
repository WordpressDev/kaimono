...
{		
	...
	function Kk_assets()
	{
		log_message('info','BackendPro->Kk_Assets : Class loaded');
		$this->CI = &get_instance();
		
		$this->CI->load->config('kk_assets',TRUE);
		
		// Load assets from config and transform them
		// into something more searchable
		$assets = $this->CI->config->item('asset','kk_assets');
		...
		$groups = $this->CI->config->item('asset_group','kk_assets');
		$this->asset_groups = $this->_transform_groups($groups);
		
		$this->caching_options = $this->CI->config->item('asset_caching','kk_assets');
	}
	...
	function icon($name, $title = false)
	{
		if($name == NULL)
		{
			log_message('error','Backendpro->kk_assets->icon : Cannot output an icon if no name is given');
			return FALSE;
		}
	...
	function load_asset($name)
	{
		...
		log_message('debug','BackendPro->Kk_Assets->load_asset : Attempting to load an asset with name : ' . $name);
	...
	{	
		...
		$this->CI->load->library('Kk_browser');		
		$this->CI->kk_browser->LoadUserAgent();
					...
					if(preg_match('|(\w+)(?:\[(?:([a-z]+)_)?([a-z]{2})(?:_([0-9\.]+))?\])\.[a-z]+|i',$file,$matches))
					{
						...
						foreach($this->assets as $name => $asset)
						{
							...
							if(	in_array($name,$this->assets_to_load) && // Are we loading this asset
								...
								$this->CI->kk_browser->DoesNameAndVersionMatch($matches[2],$matches[3],$matches[4])) // Does this browser need this file
							{
								// Add the asset to the $this->asset list
								// and then call load asset
								$md5 = md5($matches[0]);
								$this->assets[$md5] =  new Kk_AssetFile(base_url() . $dir . $matches[0]);
	...
	function load_asset_group($group)
	{
		...
		log_message('debug','BackendPro->Kk_Assets->load_asset_group : Attempting to load an asset group with name : ' . $group);
	...
	{
		log_message('info','BackendPro->Kk_Assets->_get_asset_section : Getting all ' . $section . ' assets');
		...
		foreach($this->assets_to_load as $name)
		{
			if($section == 'header' && $this->assets[$name]->type == 'css')
			{
				...
				log_message('debug','BackendPro->Kk_Assets->_get_asset_section : Adding CSS asset to load list : ' . $name);
				$css_assets[] = $name;
			}
			elseif($this->assets[$name]->type == 'js')
			{
				if(($section == 'header' && in_array($name,$this->top_js_assets)) || ($section == 'footer' && !in_array($name,$this->top_js_assets)))
				{
					...
					log_message('debug','BackendPro->Kk_Assets->_get_asset_section : Adding JS asset to load list : ' . $name);
					$js_assets[] = $name;
				}
			}
		}
		...
		if($this->CI->config->item('optimise_assets','kk_assets'))
		{
			log_message('debug','BackendPro->Kk_Assets->_get_asset_section : Optimise assets selected, checking if previous cache exists');
		...
		{
			log_message('debug','BackendPro->Kk_Assets->_get_asset_section : Optimise not selected, output assets');
		...
		log_message('debug','BackendPro->Kk_Assets->_get_asset_section : Finished getting ' . $section . ' section');
		return $output;
	}
	...
	function _output_asset($name)
	{
		...
		switch($asset->type)
		{
			...
			default:
				log_message('debug','BackendPro->Kk_Assets->_output_asset : Unknown asset type : ' . $asset->type);
				return NULL;
		}
	}
	...
	{		
		log_message('debug','BackendPro->Kk_Assets->_output_css_asset : Outputing asset with path : ' . $path);
	...
	{		
		log_message('debug','BackendPro->Kk_Assets->_output_js_asset : Outputing asset with path : ' . $path);
	...
	{		
					...
					if(preg_match('/([a-zA-Z0-9]+).(css|js)/',$file,$matches))
					{
						...
						if($name == $file_name)
						{
							...
							if($expire_time > time())
							{
								...
								log_message('debug','BackendPro->Kk_Assets->_cache_exists : Cache file found : ' . $file);
								closedir($dh);
								return $file;
							}
							else
							{
								...
								log_message('debug','BackendPro->Kk_Assets->_cache_exists : Cache found but expired, performing clean');
								$this->_clean_cache();
								closedir($dh);
								return FALSE;
							}
		...
		log_message('debug','BackendPro->Kk_Assets->_cache_exists : Cache file not found : ' . $file_name);
		return false;
	}
	...
	function _clean_cache()
	{
		...
		log_message('debug','BackendPro->Kk_Assets->_clean_cache : Cleaning out asset cache : ' . $this->caching_options['path']);
			...
			if($dh = opendir($dir))
			{
				while (($file = readdir($dh)) !== FALSE)
				{
					...
					if($expire_time <= time())
					{
						...
						if(@unlink($dir . $file))
						{
							log_message('debug',"BackendPro->Kk_Assets->_clean_cache : File deleted '" . $dir . $file . "'");
							$expire_count++;
						}
						else
						{
							log_message('error',"BackendPro->Kk_Assets->_clean_cache : Could not delete file '" . $dir . $file . "'");
						}				
					}					
				}
				closedir($dh);
	...
	function _output_cache_asset($file_name)
	{
		...
		switch($ext)
		{
			...
			default:
				log_message('error','BackendPro->Kk_Assets->_output_cache_asset : Unknown asset type : ' . $ext);
				return NULL;
		}
	}
	...
	{
		log_message('debug','BackendPro->Kk_Assets->_create_cache : Creating cache');
	...
	function _optimise_output($string, $type)
	{
		switch($type)
		{
			case 'css':
				...
				$csstidy_options = $this->CI->config->item('csstidy','kk_assets');
			...
			case 'js':
				...
				$packer_options = $this->CI->config->item('packer','kk_assets');
	...
	function _transform_assets($assets)
	{
		...
		{			
			...
			$new_asset = new Kk_assetfile($asset['file']);
	...
	function _is_loadable($asset)
	{
		...
		// Can't find asset
		log_message('error','BackendPro->Kk_Assets->_is_loadable : Cannot find any asset with given name : ' . $asset);
		return false;
	}
}

/* End of Kk_Assets.php */
/* Location: ./modules/site/libraries/Kk_Assets.php */