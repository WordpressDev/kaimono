...
{	
	...
	function Kk_site()
	{
		log_message('info','BackendPro->Kk_Site : Class loaded');
		$CI = &get_instance();
		
		$CI->load->config('kk_site',true);
		
		$this->variables = $CI->config->item("default_site_variables","kk_site");
	}
	...
	function set_metatag($name, $content, $http = false)
	{
		...
		{
			log_message("error","BackendPro->Kk_Site->set_metatag : When setting a metatag a name must be given");
			return false;
		}
	...
	function set_variable($name,$value)
	{
		if ($name == NULL)
		{
			log_message("error","BackendPro->Kk_Site->set_variable : When transfering a variable a name must be given.");
			return false;
		}

		$this->variables[$name] = $value;
		log_message('debug','BackendPro->Kk_Site->set_variable : PHP variable transfer successful: ' . $name);
	}
	...
	function set_crumb($name, $link = '')
	{
		if($name == NULL)
		{
			log_message("error","BackendPro->Kk_Site->set_crumb : When setting a breadcrumb a name must be given");
			return false;
		}
		
		$this->breadcrumb_trail[$name] = $link;
		log_message('debug','BackendPro->Kk_Site->set_crumb : Breadcrumb link "'.$name.'" pointing to "'.$link.'" created');
	}
...
/* End of Kk_Site.php */
/* Location: ./modules/site/libraries/Kk_Site.php */